...
exclusion_list = c("G215G|GGG","G247G|GGG") #G247G|GGG
...
data_5g_Loess <- data_5g_13g_combined%>%filter(LogFC_5g >=	
                                                 min(data_5g_13g_combined$LogFC_5g[data_5g_13g_combined$Mutation.type=="Syn"]), !(Mutcode %in% exclusion_list))
...
data_13g_Loess <- data_5g_13g_combined%>%filter(LogFC_13g >=	
                                                  min(data_5g_13g_combined$LogFC_13g[data_5g_13g_combined$Mutation.type=="Syn"]),!(Mutcode %in% exclusion_list))
## fixed in july 2025
...
data_3m_Loess <- data_3m_11m_combined%>%filter(LogFC_3m >=	
                                                 min(data_3m_11m_combined$LogFC_3m[data_3m_11m_combined$Mutation.type=="Syn"]), !(Mutcode %in% exclusion_list))
...
data_11m_Loess <- data_3m_11m_combined%>%filter(LogFC_11m >=	
                                                  min(data_3m_11m_combined$LogFC_11m[data_3m_11m_combined$Mutation.type=="Syn"]), !(Mutcode %in% exclusion_list))
## fixed in july 2025
...
data_5g_13g_nonsyn <- data_5g_13g_combined%>%filter(Mutation.type %in% c("Non", "Syn"),!(Mutcode %in% exclusion_list))
...
write.xlsx(edited_master_table_fix_annos,"master_table_with_new_scores_and_classifications_and_clinvar_42226.xlsx")
new_nwt = edited_master_table_fix_annos[which(!edited_master_table_fix_annos$Mutation.type=="WT_" ),]
write.xlsx(new_nwt,"master_table_with_new_scores_and_classifications_and_clinvar_non_wt_42226.xlsx")
